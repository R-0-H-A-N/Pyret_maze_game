...
data player-posn:
  | posn(x :: Number, y :: Number, moves :: Number)
end
...
init-posn = posn(init-x, init-y, 0)
...
fun place-player(p :: player-posn) -> Image:
  if num-modulo(p.moves, (game-diff)) == 0:
    I.put-image(I.square(player-size, 'solid', 'white'),
      p.x, p.y, background)
  else:
    I.put-image(I.square(player-size, 'solid', 'white'),
      p.x, p.y, background-blank)
  end
end

fun move-player(p :: player-posn, key):
  new-posn = ask:
    | key == 'up'   then: posn(p.x, p.y + player-step, p.moves + 1)
    | key == 'down' then: posn(p.x, p.y - player-step, p.moves + 1)
    | key == 'right'then: posn(p.x + player-step, p.y, p.moves + 1)
    | key == 'left' then: posn(p.x - player-step, p.y, p.moves + 1)
    | otherwise: p
  end
  new-posn
end
...
check:
  move-player(init-posn, 'up') is posn(init-x, init-y + player-step, 1)
  move-player(init-posn, 'right') is posn(init-x + player-step, init-y, 1)
end
